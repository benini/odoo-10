...
RUN DEBIAN_FRONTEND=noninteractive apt-get install -qq -y --no-install-recommends \
    libsasl2-dev libldap2-dev libssl-dev libpq-dev libjpeg-dev zlib1g-dev libxml2-dev libxslt-dev \
#   python-gevent python-ldap python-qrcode python-renderpm python-vobject python-watchdog \
    python-pip python-dev python-watchdog
#   python3-pip
...
RUN PATH=~/.local/bin:$PATH pip install --user --upgrade pip \
 && pip install --user --upgrade setuptools \
 && pip install --user -r odoo-src/requirements.txt \
 && pip install --user -r oca-it/requirements.txt

EXPOSE 8069 8071
CMD PATH=~/.local/bin:$PATH odoo-src/odoo-bin --db_host db -r odoo -w odoo \
    --addons-path "odoo-src/addons,oca-it,oca1,oca2,oca3,oca4,oca5,oca6,oca7,oca8"
